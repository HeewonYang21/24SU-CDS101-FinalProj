...
```{r}
#Violin plot for Colonies and Year
bee_df %>%
  ggplot() +
  geom_violin(mapping = aes(x = year,y = avg_max_col_1000)) +
  labs(
...
bee_df %>%
  ggplot() +
  geom_violin(mapping = aes(x = year,y = all_fules)) +
  labs(
...
bee_df %>%
  ggplot() +
  geom_violin(mapping = aes(x = year,y = count)) +
  labs(
